...
//	WipeTower::box_coordinates cleaning_box(xy(0.5f, - 1.5f), m_wipe_tower_width, wipe_area);
    ...
    for (size_t idx_tool = 0; idx_tool < tools.size(); ++ idx_tool) {
        ...
        if (idx_tool + 1 == tools.size()) {
            ...
            toolchange_Unload(writer, box , m_filpar[m_current_tool].material, m_filpar[m_current_tool].first_layer_temperature, m_filpar[tools[idx_tool + 1]].first_layer_temperature, tools[idx_tool + 1]); //Edmond
...
WipeTower::ToolChangeResult WipeTower2::tool_change(size_t tool)
{
    ...
    if (tool != (unsigned int)-1){ 			// This is not the last change.
        ...
        toolchange_Unload(writer, cleaning_box, m_filpar[m_current_tool].material, m_filpar[m_current_tool].temperature, m_filpar[m_current_tool].temperature, tool); //Edmond
    ...
    size_t tool) //Edmond
{
    ...
    // for wrong extruder.
    if (m_semm) {
        // if (new_temperature != 0 && (new_temperature != m_old_temperature || is_first_layer() || cold_ramming) ) { 	// Set the extruder temperature, but don't wait. //Edmond
